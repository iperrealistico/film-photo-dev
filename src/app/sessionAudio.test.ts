...
function createPhase(kind: PhaseDefinition['kind'], label = 'Phase'): PhaseDefinition {
  return {
    id: `${kind}-phase`,
    label,
    kind,
...
describe('sessionAudio', () => {
  it('uses dedicated drain and fill tones for transition starts', () => {
    expect(resolvePhaseStartToneKind(createPhase('drain', 'Drain developer'))).toBe('drain');
    expect(resolvePhaseStartToneKind(createPhase('fill', 'Fill stop bath'))).toBe('fill');
    expect(resolvePhaseStartToneKind(createPhase('wash', 'Wash'))).toBe('phase_start');
  });
